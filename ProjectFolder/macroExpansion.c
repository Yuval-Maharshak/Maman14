...
FILE *macroExpand(char *fileName, FILE *src) 
{
    ...
    failMalloc(content); failMalloc(line);
    ...
    while(fgets(line, LINE_LENGTH ,src)) 
    {
        ...
        {
            if (inMacro)
                printError(lineNum, "Error: macro %s defined inside macro %s",                      strtok(NULL, " "),        macroTable.head ->m.name);        /* this will cause unexpected behaviour when Error (need to check) */
            else 
            {
                ...
                if (legalMacro(word) && strtok(NULL, " ") == NULL && !search(&macroTable, word)) 
                {
                    ...
                    printError(lineNum, "Error: macro %s is poorly defined", word);     /* this will cause unexpected behaviour when Error (need to check) */
            }
        ...
        {
            if (!inMacro) 
                printError(lineNum, "Error: endmacro statement outsided of a macro");
            else if (strtok(NULL, token))
                printError(lineNum, "Error: word after endmacro");
            else 
            {
        ...
        {
            if (inMacro)
                printError(lineNum, "Error: macro %s is called inside macro %s", tempMac->name, macroTable.head->m.name);
            else 
            {
                temp = ftell(src);
