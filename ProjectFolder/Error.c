...
/*
    print error to the terminal, the call should look like printError(the line where the error occured, error format, variable to replace...)
    when first or second pass or when we finish with a file should call printError(-1, "") so that the function reset. the function will then
    return if it gotten in this pass an error.
*/
bool printError(int lineNum, char *ErrorFormat, ...) 
{
